...
class SendSummaryViaEmailService
{
    ...
    public function sendSumaryToParticipant(User $user)
    {
        ...
        $attachment = array(array('type' => 'application/pdf', 'filename' => $this->rooms->getName()  . '.pdf', 'body' => $pdf));

        $this->mailerService->sendEmail(
            $user,
            $this->translator->trans('Meeting fertig'),
            $this->environment->render('email/finishMeeting.html.twig',array('room'=> $this->rooms)),
            $this->rooms->getServer(),
            $this->rooms->getModerator()->getEmail(),
            $this->rooms,$attachment
        );
    }
}