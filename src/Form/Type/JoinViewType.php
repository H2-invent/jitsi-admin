...
class JoinViewType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if ($this->themeService->getApplicationProperties('start_dropdown_allow_app')) {
            $builder->add('joinApp', SubmitType::class, ['attr' => array('class' => 'btn btn-secondary btn-rounded btn-block p-3'), 'label' => 'label.beitretenApp', 'translation_domain' => 'form']);

        }
        if ($this->themeService->getApplicationProperties('start_dropdown_allow_browser')) {
            $builder->add('joinBrowser', SubmitType::class, ['attr' => array('class' => 'btn btn-primary btn-rounded btn-block p-3'), 'label' => 'label.beitretenBrowser', 'translation_domain' => 'form']);
        }
    }
